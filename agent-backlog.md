...
- **なぜ直っていないか（未特定・切り分けが必要）**: [[SEO-062]] の修正（pagePath 次元つき TOTAL → 次元なしクエリ）は `.gas-deploy/Code.js` に入っており、08-24 のレポートには [[SEO-063]] の新文言「判別不能（GA4しきい値）」が出ている＝**同一ファイルの新しい版が本番に入っているはずなのに、直帰率だけ直っていない**。想定される切り分け:
  1. 本番 GAS が `.gas-deploy/Code.js` の**部分的な版**（貼り付け漏れ・GAS側に複数ファイルが残存 等）
  2. 修正自体にバグ（`parseTotals` が `rows[0]` を読む前提と、実際の API レスポンス形の不一致 等）
- **切り分けの好機（既に条件が揃っている）**: 2026-08-25 の作業中に `deploy-gas.sh` が実行され、**現在の `.gas-deploy/Code.js` が本番へ push 済み**（送信内容はコミット済みと同一・blob `c489f4bc`）。したがって**翌朝 2026-08-26 のレポートが切り分けになる**:
  - 直帰率が 30〜40% 台で出る → 原因は①（それまでの本番が古い/部分的な版だった）。本チケットは done
  - 直帰率が 90% 前後のまま → 原因は②（修正自体のバグ）。`parseTotals` / `totalsRequest` を実レスポンスで検証する
- **acceptance**:
  1. 2026-08-26 の日次レポートの直帰率を GA4 の同日実測（トラフィック獲得の合計行のエンゲージメント率）と突き合わせ、**±10pt 以内**に入ることを確認する（合否は自己申告ではなく両者の実測値で判定・制約10）
  2. ズレが残る場合、`.gas-deploy/Code.js` の `totalsRequest` の実レスポンスを GAS のログに出して `rows[0]` 前提が正しいか確認し、必要なら `metricAggregations` を付けて `totals[0]` を読む形へ直す
  3. 恒久策: 直帰率の突合を**人の目視に依存させない**。`fetch_ga4_views.js`（独立系・次元なし）の値と、GAS レポートが出した値を機械で比較し、乖離が閾値を超えたら検知する経路を作る（[[ISSUE-084]] 原則2: 通知は out-of-band へ。既存 `gas-deploy-watchdog.yml` と同じ思想）
  4. 直るまでの間、直帰率を根拠にした画面改修の助言は採用しない（本日の [[SEO-072]] 起票時の判断と同じ）
