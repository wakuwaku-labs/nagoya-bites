...
- **調査で判明した事実**:
  - `index.html:1683-1699` の常時表示 `.scene-nav`「シーンで探す」は 宴会・忘年会／接待・会食／デート／女子会／記念日／個室 の6つで、**一人飲みが無い**
  - 一方、検索側の概念辞書には既に `nbC('hitori','scene','一人・少人数', …)`（`index.html:8312` 付近）が実装済み＝ `suggestSearch('一人飲み')` はそのまま機能する見込み（新ロジック不要）
  - `data/gsc_metrics.json` の実測: `名古屋 一人飲み` 303表示/36クリック/5.5位、`名古屋駅 一人飲み` 179表示/10.5位 ほか「一人飲み」系が discovery クエリの最大群
  - GA4 日次で `features/nagoya-solo-dining` が閲覧TOP1を反復（09-20: 6回 / 09-24: 5回 / 09-25: 10回）
  - 店舗ページ（stores/*.html 2,403本）は既に solo-dining へリンク済み＝助言後半の「店舗詳細への導線」は増分が小さいため今回は範囲外
- **brand-filter**: ✅ 適合 — Moat「シーン別の専門性」をトップの第一導線に反映するだけで、順位操作・広告・新データ生成を伴わない。既存の実在データ（hitori 概念の検索）へ入口を付けるのみ
- **acceptance**:
  1. `.scene-nav` の「シーンで探す」行に「一人飲み」チップを追加（`suggestSearch('一人飲み')`）。既存チップと同じ `.scene-chip` を使い、新規CSS・font-sizeリテラル追加なし（制約12）
  2. 押下で hitori 概念の検索結果が出ることをプレビューで確認（0件にならない）。スマホ幅で行が横スクロールのまま崩れないこと（nowrap 方式を維持）
...
- **① 焼肉 三郎 原店（`stores/J004678178.html`）— 検索されている語をtitleもdescriptionも一度も使っていない**
  | 事実 | 根拠 |
  |---|---|
  | クエリ「焼肉 三郎 原店 名古屋市 レビュー」が28日で**785表示・7クリック・CTR0.89%・9.5位**（地名「名古屋市」を含むため[[SEO-061]]のgeoless除外に該当せず、正規のctrFix対象） | `data/gsc_metrics.json` `pageQueries` |
  | 同ページは他に「...レビュー」を含むクエリを**5種**持つ（レビュー/名古屋市レビュー/原店レビュー 等、計約100表示） | 同上 |
  | サイト全体でクエリに「レビュー」を含む行は7件・881表示のうち、**785表示（89%）がこの1ページに集中**（他ページにも薄く存在するが `pageQueries` は1ページ15件上限のため未捕捉分がある） | `pageQueries` 全走査 |
  | 現在の `<title>` `<meta description>` はどちらも「レビュー」という文字列を含まない（予算・エリア・ジャンル・Google評価は入っている） | `grep -o '<title>[^<]*</title>\|<meta name="description"[^>]*>' stores/J004678178.html` |
  | 同ページは page×query 単位でも rankPush 対象（10.3位・upside 14.9クリック推定）に重複計上されている＝同一の欠落が2つの機会バケットに跨って出ている | `data/gsc_opportunities.json` `byPage.rankPush[1]` |

- **② cm（センチ）伏見店（`stores/J004660861.html`）— 4.5〜6.1位という好順位なのにCTRがほぼ0%**
  | 事実 | 根拠 |
  |---|---|
  | 「センチ 伏見」116表示・0クリック／「cm 伏見」69表示・2クリック／「cm（センチ）名古屋市」40表示・0クリック／「伏見 センチ」28表示・0クリック — いずれも**4.5〜6.1位**（検索結果1ページ目上部） | `data/gsc_metrics.json` `pageQueries` |
  | 同順位帯の期待CTRは5〜8%程度だが実測はほぼ0%。[[SEO-058]]の「順位相応だから直らない」パターンとは異なる（あちらは18〜26位） | `data/gsc_opportunities.json` `benchmarks` |
  | `data/stores.json` の当該店レコードに **`Google評価`・`口コミ数` フィールドが存在しない**（値が空ではなく、キー自体が無い） | `data/stores.json` の J004660861 レコードを他店（例: 三郎=Google評価3.5/口コミ13）と比較 |
  | `gen-store-pages.js` の `buildTitleFacts()` は `score && reviews>=5` のときだけ「Google★」を title に付与する仕様。評価データが無いため、このページは title/description のどこにも信頼シグナル（評価・口コミ数）を一切出せていない | `gen-store-pages.js:423-436` |
- **仮説（未確定・要検証と明記）**: 検索結果の隣にはGoogleのナレッジパネルや他媒体の評価が並ぶことが多く、当ページだけ評価スコアが一切出ないことが「情報が薄い」と判断され素通りされている可能性がある。ただし同型の他店との比較検証はしていないため、**acceptance①で先に検証してから**手を打つ（決めつけて実装しない＝品質ゲート原則5）

- **acceptance**:
  1. ①②とも個別HTMLを手で直さない。`stores/*.html` は `gen-store-pages.js` の生成物のため、テンプレート/データ側を直す（[[SEO-086]] acceptance②と同じ理由＝手編集は次回生成で消える）
  2. **①**: `gen-store-pages.js` の `buildDescription()`（`stores/J004678178.html:443`付近）に、口コミ数を持つ店の description で自然に「レビュー」または「口コミ」という語が検索クエリと一致する形で出現するよう調整する。特定1店への決め打ちではなく、既存の `reviewTrustTier`/口コミ数ロジックに乗せて**全店で一般化**すること（この店だけの特例処理を禁止）
  3. **①の対照実験**: 変更前後で該当クエリ群のCTR/クリック数を次回GSC更新（翌週以降）で比較する。改善しなければ「レビューという語の有無は効かない」という反証結果として記録し、それ以上は追わない（体感で判定しない・制約10）
  4. **②**: まず `GOOGLE_MAPS_API_KEY` 経由で当該店（placeId解決含む）のGoogle評価・口コミ数を再取得できるか試す（`scripts/fetch_manual_store_photos.js` 等既存の取得経路を流用。新しい取得ロジックを増やさない）。取得できれば通常どおり title/description に反映されるはずなので、それ自体が施策になる
  5. **②で取得できない場合**: 同一順位帯（4〜7位）で評価データを持つ他の店舗ページを3〜5件サンプリングしCTRを比較し、「評価データの有無」が本当にCTR差の説明変数か検証する。有意差が確認できない場合は原因不明のまま記録し、取り繕った修正はしない（品質ゲート原則1）
  6. 制約1・2・5・12（単一ファイル維持・LOCAL_STORESパターン・フィルタ等非破壊・DSN-001）に抵触しない（今回は生成器のテンプレート文言のみの変更でUI部品は増えない）

- **関連**: [[SEO-058]]（トップページの低順位はStrategic Skip寄り・本チケットは対照的に「好順位なのに機会損失」の事例）／[[SEO-061]]（地名なしクエリのgeoless除外ロジック）／[[SEO-086]]（個別HTML手編集禁止の同型判断）／[[SEO-095]]（title/description生成ロジックの正本）
...
- **category**: SEO
- **owner**: Builder
- **source**: SEOアドバイス(LINE) 2026-09-21 原文「訪問者45人に対し、店舗詳細を開いたのは4回と回遊が少ないです。特にトップページからの回遊を促せていません。👉 index.html の全店舗一覧で、人気特集（nagoya-solo-dining, nagoya-kaoawase-washokuなど）で紹介されている店舗に『掲載特集』などのラベルを追加し、詳細モーダルへの誘導を強化しましょう」
- **brand-filter**: ✅ 適合 — Moat「構造化DB 4,915店 × 特集68本 × 日次ジャーナルの三層編集」を、**現在は片方向にしか繋がっていない**（特集→店舗はリンク済み／店舗→特集は0本）。カード上で「この店は編集部が◯◯特集で選んでいる」と示すのは、大手ポータルが構造的に書けない解釈層の可視化であり、順位操作・広告・送客手数料のいずれにも触れない（制約7・8 非該当）。装飾の追加ではなく**既存の編集判断の露出**である点が Strategic Skip「女性向け装飾演出」と異なる
- **検証できる事実（制約10）**:
  | 事実 | 根拠 |
  |---|---|
  | `index.html` にこの種のラベルは**1箇所も存在しない**（`掲載特集` / `feature-badge` / `featuredIn` の出現数 0） | `grep -n "掲載特集\|feature-badge\|featuredIn" index.html` |
  | 特集68本に名前が出る LOCAL_STORES の店は **401店（全4,915店の8.2%）**、うち **126店が2本以上**の特集に載る | `features/*.html` × `data/stores.json` の店名機械照合 |
  | 店舗→特集の逆引きリンクは `stores/*.html`（5,548枚）にも無い | `grep -l "掲載特集" stores/*.html` が0件 |
  | 店舗↔特集の対応は既に決定的に計算できる（新しい判定基準を足す必要が無い） | `scripts/audit_feature_stores.js` が同じ照合で「実在不明掲載店」を日次検出している |
- **助言の前提のうち採らなかった部分**: 「回遊が少ない」の根拠は訪問45人／店舗詳細4回＝**単日の分母45**であり、多寡を論じられる母数ではない。加えて特集・ジャーナル・店舗ページ側の詳細クリックは [[SEO-084]] / [[SEO-072]] で計測を足した直後で、実数は過少に出る。**本施策は「回遊数値が低いから」ではなく「三層編集の片方向リンクを閉じるから」採用する**（数値は効果測定にのみ使う）
- **acceptance**:
  1. 店舗↔特集の対応表を `scripts/audit_feature_stores.js` と**同じ照合ロジック**で生成する（新規の判定基準を作らない／`scripts/lib/` に共有化して生成器と監査が1本を通る）。対応が取れない店にはラベルを出さない（取り繕わない）
  2. `index.html` の店舗カードに、掲載特集がある店だけラベルを描画する。ラベルは特集ページへの実リンクとし、クリックを既存のイベント名規約で計測する（[[SEO-084]] と同じ形。計測が無いと効果を検算できない）
  3. 2本以上に載る126店の表示ルールを決める（最大N本・溢れは「ほか◯本」）。**リンク先は必ず実在する特集ファイル**で、`node scripts/audit_feature_stores.js` の「リンク切れ 0」を維持する
  4. 新規UI部品のため Designer レビュー（`agents/designer.md` QA-5）と `node scripts/audit_design_system.js --check` の通過を公開条件にする（制約12・DSN-001）。font-size リテラルの直書き禁止・トークン `var(--fs-*)` を使う
...
- **brand-filter**: ✅ 適合（振替採用）— literal な打ち手（色/サイズを大きく変更＋「今すぐ電話」文言）は却下し、実測で確認できた構造的欠落へ振り替える。**却下の根拠**: ① 「視認性が低い」という前提が検証されていない。index.html のモーダル電話ボタンは placeId 保有 4,722/4,911店（96.2%）で描画され（`index.html:9884` `nbCallBtnHtml`）、タップ上限は20回/日/ブラウザ（`index.html:9906`）で通常利用の制約にならない。② 色/サイズの直変更は DSN-001（制約12）に抵触し、Designer レビューと `audit_design_system.js --check` を経ずに実施できない。③ 「31回 vs 2回」の比較自体が面積の違いで、予約ボタンはカード・モーダル・店舗ページ5,618本の3面に出るのに対し、電話ボタンはモーダル内の1面にしか存在しない（週の店舗詳細オープンは93回＝分母が45分の1）。**採用に振り替えた事実**: Moat の中核である編集部推薦店ほど行動導線が細っている。
- **trend**: 電話ボタン 2回/週（週次レポート実測）。ただし週次の「予約31回」との比較は分母が異なるため、本チケットは比率ではなく**導線の有無**を扱う。
- **実測（2026-09-21・`data/stores.json` 4,911件と `stores/*.html` 5,618本を全件走査）**:
  1. `stores/*.html` のうち `tel:` リンクを持つページ: **0本 / 5,618本**。`gen-store-pages.js:754-755` が出すCTAは「ホットペッパーで予約」「Googleマップ」「Instagram」の3種のみで、電話導線は生成対象に無い
  2. `data/stores.json` の全69フィールドに電話番号カラムが存在しない（電話番号は index.html が Places Details を**タップ時に都度取得**する設計＝`nbCallStore`）
  3. ホットペッパーIDも食べログURLも持たない店 = **115店**。この115店の静的ページは実質 Googleマップ1本しか行動導線が無い
  4. うち **placeId 保有 109店**（電話導線を出す材料がある）／うち **編集部推薦・手動キュレーション 67店**
- **acceptance**:
  1. `gen-store-pages.js` に電話CTAを追加する。**対象は「予約導線を持たない店（ホットペッパーID・食べログURL がいずれも無い）かつ placeId 保有」の109店に限定**して開始する（全5,618本への一律追加はしない＝Places Details の課金面を不必要に広げないため）
  2. 取得方式は index.html と同一にする（`nbCallStore` と同じく **タップ時のみ** Places Details を呼び、Maps JS も遅延ロード。ページ表示だけでは1回も API を呼ばない）。日次タップ上限（現行20回/日）も同じ判定を共有し、実装を二重化しない
  3. 見た目は既存 `.link-btn` 系の共通クラスを再利用し、新規 font-size リテラル・`:root` 再定義を足さない。**公開条件として `node scripts/audit_design_system.js --check` の出力が着手前ベースラインから悪化していないこと**（現状 `stores/*.html` 由来の既存違反で exit 1 のため、前後比較で判定する。[[SEO-099]] と同じ手順）
  4. Designer（`agents/designer.md`）のレビューを通す（DSN-001・新規UI部品の追加に該当）
  5. 効果は `cta_call_click` の発火数を実装前4週 / 実装後4週で比較する。体感ではなく GA4 実測で判定する（制約10）。**分母を必ず併記する**（対象109店のページビュー数）。分母が小さく判定不能なら「判定不能」と記録し、増えたことにしない
  6. 電話番号が取得できなかった場合にボタンが無言で死なないこと（index.html 既存実装と同じくメッセージを出して復帰する）
- **関連**: [[SEO-095]]（店舗ページのtitle/meta最適化＝この面に検索流入が来ている前提）／[[SEO-013]]（予約CTAの視認性・done）／[[ISSUE-104]]（ホットペッパーID非保有の手動キュレーション店の口コミ収集）／[[DSN-001]]（デザインシステム通過が公開条件）
- **注記（要検討）**: 115店のうち placeId も無い6店は、この施策でも導線ゼロのまま残る。実在検証の観点からも別途 DataKeeper 側の棚卸し対象になりうる。
...
- **2026-09-19 訂正（日次SEOトリアージで実地再検証・このチケットは着手不要）**: 本チケットの前提「09-17・09-18 の日次レポートが届いていない」は**誤り**だった。オーナーが GAS の実行ログを見に行く必要はない。検証できる事実（Gmail `日次レポート in:anywhere newer_than:8d`・誰でも再現可能）:
  - 09-11 / 09-12 / 09-13 / 09-14 / 09-15 / 09-16 / 09-17 / 09-18 の **8日分が欠番なしで存在**し、**全て 23:07 UTC（＝翌 08:07 JST）送信**で時刻も安定している。GAS のトリガーも実行も止まっていない
  - 本チケットが「0件」と観測した原因は**探した時刻**。日次レポートは JST 08:07 に届くため、それ以前に検索すると当日分はまだ存在しない。ISSUE-129 は「まだ送られていないメール」を「止まった」と解釈していた（＝不在の証明を、時刻を確認せずに行った）
  - 2026-09-19 の本ルーチンは 09-18 分を正常に取得し、`check_gas_deploy_health.js --record` で `verdict: deployed` を記録済み（直帰率 レポート28% vs GA4実測28.8%・差0.8pt）
  - 残る本物の問題は「メールが届かないこと」ではなく「**届いたメールが数日で TRASH に移ること**」で、そちらは [[SEO-103]] に分離して起票した
- **教訓（CLAUDE.md 制約10 の適用範囲）**: 「無いこと」を根拠にする判定は、**探した窓が対象の生成時刻をまたいでいるか**を先に確かめないと成立しない。今回は P1 として起票され、オーナー本人の GAS 調査を要求する寸前だった（Notion 同期の create キューに載っていた）
...
- **解決内容（2026-09-16）**: オーナーが「index.html のモーダルでは同じ投稿の写真が表示される」とスクリーンショットで提示。両者のコードを比較したところ、**index.html のモーダル（`#mig-container`・`setupLazyIg()`）は公式ウィジェットを使わず、`<iframe src="https://www.instagram.com/p/<code>/embed/">` を固定高さ(560px)で直接埋め込むだけ**（postMessageによる動的リサイズに依存しない）という、全く別の実装だった。これは実機で表示確認済み（オーナー提供スクショ）。`gen-store-pages.js` の埋め込み実装を、この実績ある方式に統一して解決した。CSS Gridの撤去（[[ISSUE-128]]調査中の中間対応）は実は原因ではなかった（direct-iframe化が効いた後にGrid/Flexを再導入しても問題なく動作している）
- **副産物として判明した別のギャップ（フォローアップ・下記メモ参照）**: 実際に写真が表示されるようになったことで、オーナーの目視により「本文キャプションでは通っているが実際の画像は販促バナー」の投稿が1件見つかった（`data/ig_post_policy.json` の判定はキャプション本文だけを見るため、画像そのものの中身は検証できない・journal記事のヒーロー写真ゲート[[ISSUE-090]]と同じ死角）。該当投稿は当該店（J004537202）のデータから人力で除外済み。自動判別の実装は過去に画素解析を試みて断念した前例（`journal_photo_policy.json` 参照）があるため、今回も自動化はせず「表示された結果を人が見て気づいたら外す」運用を前提とする
- **旧・調査ログ（解決前の記録として保持）**:
- **detected**: 2026-09-15（[[複数投稿表示機能]] の本番確認中に、オーナーから「表示されないです」と報告。調査の結果、今回変更していない既存の単一投稿ページでも同一症状を確認し、本件を独立の課題として切り出した）
- **category**: 表示 / 外部連携（Instagram embed）/ **写真ソース優先順の前提崩壊**
- **owner**: Builder / DataKeeper（写真ソース優先順の運用は両者共管・CLAUDE.md参照）/ Strategist（優先順再設計が必要なため）
- **source**: 本セッションでの直接調査（本番URLをブラウザ・puppeteerで実測、レスポンスヘッダーを直接確認）
- **確定した原因（制約10・誰でも同じ手順で再現・検算できる）**:

  ```
  curl -sS -D - -o /dev/null \
    -H "Referer: https://nagoya-bites.com/stores/J004537202.html" \
    "https://www.instagram.com/hakata_kawaya.meieki/p/DWdmsGrkqNH/embed/?cr=1&v=14&wp=326&rd=https%3A%2F%2Fnagoya-bites.com"
  ```
  のレスポンスヘッダーに **`x-frame-options: DENY`** が付与されている。これはブラウザに「このページを一切iframe内に表示させるな」と強制する標準セキュリティヘッダーで、クライアント側のコードでは回避不可能（当サイトに限らず、iframeで埋め込もうとする全サイトが同じ制約を受ける）。

- **原因特定の道筋（切り分けの実測）**:

  | 検証 | 結果 |
  |---|---|
  | 埋め込みURLを**トップレベルナビゲーションで直接開く**（puppeteerでJS実行込み・ログイン無し） | 本文・画像とも正常に取得できる（`hasCaption:true, hasImg:true`）。ログイン済みCookieを使っても結果は同一 |
  | **同じURLを nagoya-bites.com の iframe として埋め込む**（本番ページをpuppeteerでフル実行） | `height:0` のまま。サードパーティCookieを明示的に許可/ブロックのどちらでも結果は変わらない（＝Cookie起因ではない） |
  | 該当リクエストのレスポンスヘッダーを直接確認 | `x-frame-options: DENY` を検出 |
  | 既存の単一投稿ページ `https://nagoya-bites.com/stores/J000015064.html`（本タスク無変更）でも同一症状 | 全店舗共通の問題であり、今回のコード変更とは無関係と確定 |
  - 当初「Instagram側の匿名アクセス制限（`require_login`）が原因」と推定したが、これは**誤り**だった（トップレベルでは無ログインでも成功したため棄却）。`require_login` は別エンドポイント（`web_profile_info` API・投稿一覧取得用）の話で、本件（iframe埋め込み失敗）とは別の制限
- **「なぜDENYが付くのか」は未確定（2026-09-16 追加調査・一次情報にあたった結果）**:
  - Meta公式の Instagram Platform oEmbed ドキュメント（developers.facebook.com）を直接確認したが、**iframe埋め込みの廃止やX-Frame-Options追加を明記した公式発表は見つからなかった**
  - Web検索で見つかったのは (a) 2020年導入の「oEmbed APIアクセストークン必須化」を2026年6月にMetaが撤回したという記事（生JSON APIの話で `blockquote.instagram-media` 方式とは別メカニズム）、(b) 「Reelsプレイヤーiframeのクロスオリジンポリシーが強化され"適切な設定"がないサイトで失敗するようになった」という記述（出典は個別ブログで一次情報ではない）。どちらも本件を確定的に説明する一次情報ではない
  - 残る2つの仮説（切り分け未了）:
    1. **意図的な仕様変更**として全サイト一律にDENYを付与している（ただし影響が甚大な割に一次情報が見当たらないのは不自然）
    2. **ボット検知による選択的ブロック**: 検証に使った curl / puppeteer が自動化ツールとして検知され、その結果だけDENYが返っている可能性。本物の人間の通常ブラウザでは異なる挙動の可能性がある
  - ただし**オーナー自身の実機でも同一症状**が報告されているため、仮説2だけでは説明しきれない面がある。両仮説とも単独では全ての観測事実を説明できておらず、確定させるには追加調査が要る
- **影響範囲だった**: `blockquote.instagram-media` + `embed.js` 方式で埋め込んでいた**全店舗ページ**（`__igVerified` が true の全件・[[ISSUE-089]] 所有者検証通過数を参照）。direct-iframe方式への統一により解消
- **戦略的インパクトは解消**: CLAUDE.md「写真ソース優先順」の優先1（公式Instagram embed）は実装方式を変えただけで引き続き機能している。優先順の記述変更は不要になった
- **残タスク**: 今回は J004537202（複数投稿表示機能の対象店）のみ direct-iframe 方式に更新済み。他の全店舗（単一投稿）は次回 `node gen-store-pages.js` 実行時（build.ymlの日次自動実行に乗る）に自動で新方式へ切り替わる見込み。手動での一括再生成は不要（生成器を直しただけで以降の生成物が全て新方式になるため）
- **関連**: [[複数投稿表示機能]]（本課題発見のきっかけ）/ [[ISSUE-089]]（Instagram埋め込みの所有者検証）
- **files**: `gen-store-pages.js`（`.ig-photos`/`.ig-embed-box`/`.ig-embed-grid` セクション・iframe直接埋め込みへ実装変更）, `stores/J004537202.html`（同上を適用・不適切な投稿2件を人力除外）
...
- **2026-09-21 追記（未反映が継続している直接証拠・SEOループより）**: 本日の日次レポート（2026-09-20分）と週次レポート（09-13〜09-19）の**両方**が、失効した前提（「docs/daily-posts/ にあるSNS投稿原稿を投稿せよ」）を含む助言を再び出力した（日次=3件中1件／週次=3件中1件）。`node scripts/check_gas_deploy_health.js` も `反映待ちの修正: 2件 — SEO-092, SEO-093` を報告している。**GASへのデプロイはオーナー本人の操作**のため本ループでは解消できない。停止（2026-09-05）以降、この失効前提の助言に消費された助言枠は本日分を含めて増え続けている。


- **2026-09-23 追記（8回目・「検知が届かない」穴を塞いだ／日次SEOトリアージ）**: 本日の日次レポート（2026-09-22分）の助言③が再び「`docs/daily-posts/` にあるSNS投稿原稿を、InstagramやXで毎日投稿し」と出力し、停止（09-05）後**8回目**（09-07/08/10/12/13/16/20 に続く）。助言そのものは前提失効として却下した（`data/seo_advice_log.json` 2026-09-23）。
  - **本日判明した真の欠陥は再発回数ではなく、再発が out-of-band に届かないことだった**。既存痕跡 `sns_daily_posts_reuse_old_label` は**ルールベース保険側の固定文** `docs/daily-posts\s*の原稿を流用` のみを見ており、AI生成側は毎回言い回しを変える（本日は「にあるSNS投稿原稿を、InstagramやXで毎日投稿し」）ため**原理的に捕捉できない**。結果、8回のうち大半が `deployed` と記録され、`gas-deploy-watchdog.yml` は**一度も鳴らなかった**。2026-09-16 追記が「痕跡にAI生成側の言い回しを足すかは実装判断」として保留したまま7日間放置されていた箇所（ISSUE-084 原則3「気づけるはずを検知と数えない」の再発）。
  - **対応（acceptance④の実施）**: `data/gas_deploy_policy.json` に痕跡 `sns_daily_posts_path_reference`（not_deployed / scope=body / pattern=`docs/daily-posts`）を追加。**言い回しではなくパス文字列そのもの**を見ることで、AI生成・ルールベースのどちらの経路でも1本で捕捉する。新コードは「SNS原稿はサイト外運用・リポジトリ内には無い」を前提とするためこのパスを助言に出さない＝新コードでは原理的に出力されない文字列（制約10。誰でも当日のメールを Ctrl+F して検算できる）。反映されれば出力が止まり自動で静かになる（ISSUE-084 原則6）。判定器（`scripts/lib/gas_deploy_trace.js` / `check_gas_deploy_health.js`）は**一切触っていない**（CLAUDE.md「閾値変更はJSONで行いスクリプトは触らない」）。
...
- **priority**: P2 → **status**: wont_fix（2026-09-14 診断完了・③需要なしで着地。理由は下記「診断結果」参照）
- **detected**: 2026-09-12
...
- **診断結果（2026-09-14・`data/gsc_metrics.json` 2026-08-17〜09-13 の28日分・全て検証可能な実測値）**:

  | 検証項目 | 実測 | 出典 |
  |---|---|---|
  | 6本の GSC 実績（`pages`＝上位500ページ中） | `nagoya-settai-lunch` 2表示/0クリック・`private-room` 19表示/0クリック(平均順位18.6)・`nagoya-meieki-business-dinner` 2表示/0クリック。`nagoya-settai-concierge`／`nagoya-settai-secret`／`settai-guide` は上位500ページにも不出現＝0表示 | `data/gsc_metrics.json` `pages` |
  | 6本の per-page クエリ内訳（`pageQueries`） | **6本とも1件も無い**。`pageQueries` は上位14ページ（一定のクリック/表示を稼いだページ）にしか出ない仕様で、6本はどれもその閾値にすら届かない | 同 `pageQueries`（全14件を全数確認） |
  | 「接待」「会食」を含むクエリ | サイト全体で28日に観測された distinct クエリ **3,998件**中、上位300件（`queries`）を全数走査して**0件**。「顧客」「出張」「ビジネス」「商談」「取引先」「社用」「来客」「ゲスト」「おもてなし」「宴会」の派生語でも**0件** | `data/gsc_metrics.json` `queries`（300件全数） |
  | 「個室」を含むクエリ | 6件のみ、うち5件は**すべて「顔合わせ」意図**（両家顔合わせ・結納のランチ個室探し）で、`nagoya-kaoawase-washoku.html`（接待6本とは別の既存特集）が単独で受けている。残り1件は特定店名「コモレビ」の指名検索 | 同上 |
  | 比較対象 | `nagoya-solo-dining` 単独 3,031表示/152クリック（全サイト最大級の資産）。接待6本の合計表示（23前後）はその**0.8%未満** | `data/gsc_metrics.json` `pages` |
  | 内部リンク供給（`grep -rl` で inbound 数を実測） | `private-room` 263本・`nagoya-settai-lunch` 64本・`settai-guide` 62本・`nagoya-settai-secret` 17本・`nagoya-meieki-business-dinner` 4本・`nagoya-settai-concierge` 1本。**最も内部リンクが厚い private-room（263本）でも0クリック**＝内部リンク不足が主因ではない | `grep -rl "<file>.html" index.html features journal stores` |
  | インデックス状態 | 6本とも `sitemap.xml` に掲載・`noindex` タグ無し＝クロール/インデックス阻害ではない | `sitemap.xml` ＋各ファイルの `grep noindex` |
  | 「顔合わせ」KWは既に手当て済み | `data/journal_seo_keywords.json` の `scenes` に「顔合わせ」→`nagoya-kaoawase-washoku.html` は SEO-011（2026-06時点）から既存。acceptance③の「別の語に需要があれば追加」は**追加不要**（既に存在） | `data/journal_seo_keywords.json`／`git log -S 顔合わせ` |

  **結論＝③（需要そのものが無い）**。①カニバリは確認できず（そもそも per-page クエリ内訳に乗るだけの表示すら無く、競合しようがない）。②「別の語で需要がある」の候補だった「顔合わせ」は既に別の実在特集で手当て済みで新規対応不要。「接待」「会食」という語そのものへの検索需要が Google 検索（web検索）には実測上存在しない。2026-06 の SEO-005/006/007（当時のLINE助言「名古屋 接待 個室」を勝ち筋と見て特集を複数制作した経緯）は、3ヶ月後の実データで**前提が誤りだったと判明**した。
  - 注記: GSC は web検索のみを計測し Google マップ内検索は計測範囲外のため、「接待ニーズは地図・口コミ経由で完結し検索に現れない」という可能性は本データでは肯定も否定もできない。ただし acceptance 4 の判定基準（「取れていない理由が需要そのものが無いなら撤退を認める」）に対しては、**観測可能などの語でも需要が確認できない**という事実だけで十分に足りる。
- **実施内容（新規作成・既存ページの改変とも無し）**:
  1. 本チケットを `wont_fix` として閉じる（acceptance 4 準拠）。features/6本は一切変更しない（統合・差別化のトリガーとなるカニバリが確認できなかったため、内部リンク削除や本文変更もしない＝掲載中の実在店への導線は無傷のまま）
  2. `data/journal_seo_keywords.json` への追加KWは無し（「顔合わせ」は既存のため対応不要と確認しただけ）
  3. `data/seo_advice_log.json` に本診断結果を追記し、同種の「接待・個室特集を増やす」助言が将来再度届いた場合に同じ調査を繰り返さないよう理由を記録
- **効果測定方法（今後この結論を再検証する場合の基準線・acceptance 6）**: 何もしない結論のため「施策の前後比」は無いが、**再検証の基準線**として以下を記録する。再度「接待/個室特集を作るべき」という助言が来た場合、この数値が変化していない限り同じ結論を維持する。
  - discovery行のベースライン（2026-09-14・`node scripts/gsc_query_intent.js`）: discovery 38クエリ / 表示1,692 / クリック99 / CTR 5.85% / 平均順位7.8 / 表示シェア30.4%（＝「接待」クエリはこの中に1件も含まれない）
  - 対象6本の表示合計ベースライン（28日・2026-08-17〜09-13）: 23前後（`pages` 実測分）/ クリック0
  - 次回チェック方法: `node scripts/gsc_query_intent.js` の discovery 例に「接待」「会食」語が出現するか／`data/gsc_metrics.json` `queries` に該当語が上位300入りするかを見る。入るようになって初めて再着手を検討する
...
- **2026-09-14 追記（診断・実施内容）**:
  - **acceptance 1（クエリ実態の確定）**: `data/gsc_metrics.json`（現時点のウィンドウ 2026-08-17〜09-13）でも当ページは `pageQueries` に**登場しない**。`pageQueries` は `fetch_gsc_metrics.js` の `PAGE_QUERY_FOCUS_PAGES=15`（表示・クリック上位15ページ＋ホーム）に限定した仕組みで、当ページ（クリック2）はその対象に入らない。全サイト上位300クエリ（表示回数順）を確認したところ「ひつまぶし」を含むクエリは**「名古屋 国産うなぎ ひつまぶし」1本のみ（表示1・クリック1・8位）**で、ボリュームはごく僅か。つまり当ページの表示306件は、単一の代表クエリに集約せず**大量の長尾クエリに分散**していると判断できる（無関係クエリで表示が水増しされているのではない＝**題材の再設計は不要**、タイトル/見出しのKWは既に妥当）。3ページ目滞留の原因は「間違ったKWで出ている」ことではなく、**同一テーマの競合が極めて厚い中でのコンテンツの実力・内部リンクの不足**と判断し、rank_push（内容強化・内部リンク）で対応した
  - **acceptance 2（内容強化）**: 掲載9店それぞれに `data/stores.json` の実データ（価格帯・Google評価・口コミ数・席数・宴会収容）を追記し、store-meta と業界人ポイントを厚くした。その過程で**事実誤り1件を発見・訂正**: 雅MASA（J003806845）の会席コース価格を本文が「8,000〜15,000円」としていたが、実際の HotPepper 価格帯は「15,001〜20,000円」だった（constraint 10 違反状態）。また「3,000円以下のひつまぶしは警戒」という冒頭の一般論が、実は自店の掲載9店中3店（備長エスカ店・稲生エスカ店・鰻のノリスケ、いずれも2,001〜3,000円台で口コミ487〜1,178件・Google4.3〜4.7）と矛盾していたため、両立する説明を追記した。掲載店の差し替えは行っていない（9店とも LOCAL_STORES 実在店で内容の薄さが差し替えを要するレベルではなかったため）
  - **acceptance 3（内部リンク）**: `scripts/refresh_journal_related.js` の `TOPIC_FEATURES` に**根本原因のバグ**を発見: 正規表現 `/ひつまぶし|うなぎ|鰻/` が1エントリに統合されており、ひつまぶしをテーマにしたジャーナル記事も含め常に `nagoya-unaju.html`（うなぎ全般10選）へのリンクしか生成されず、**より専門的な `nagoya-hitsumabushi.html` への内部リンクが自動生成の仕組み上ずっと1本も発生していなかった**（`journal/2026-06-09-hitsumabushi-touga-nagono.html` はひつまぶし単体がテーマの記事なのに、当ページへのリンクを持っていなかった）。正規表現を「ひつまぶし」単独判定を先に置く形に分離し、`node scripts/refresh_journal_related.js` を実行して是正（影響1ファイル。無関係な既存ドリフト1件を検知したが本チケットの対象外のため別途フォローアップとして起票し、コミットからは除外した）。加えて、サイト最大資産 `features/nagoya-solo-dining.html`（28日145クリック=26.3%）の関連リンクにも当ページを追加。当ページから掲載9店 `stores/*.html` への導線・`add_feature_top_cta.js` のTOP-CTAは既存実装で問題なし（`--check` で `"status":"unchanged"` を確認）
  - **acceptance 4（Bing/GSCの限界）**: 本施策の起点となったLINE助言は「Bing流入36%」を根拠にしていたが、**GSCはGoogleの掲載順位・クエリしか観測できず、Bing側の効果はこのチケットの実装では測定できない**。`node scripts/search_channel_metrics.js --report` の実測（2026-09-14時点・直近30日）では検索発見1,014セッション中 Google 40.6%・Bing 30.8%・Yahoo! 17.5%・生成AI 9.8%で、Bingは無視できない比率を占め続けている。Bingのクエリ別実データ取得は [[SEO-067]]（オーナー本人操作待ち・blocked）が解消するまで観測外であり、今回の効果測定はGoogle側（GSC）の前後比のみで行う
  - **acceptance 5**: `node scripts/audit_feature_stores.js` 実行結果は実在不明3件（fathers-day-2026.html / nagoya-kaoawase-washoku.html / nagoya-ramen.html）のみで、いずれも本チケット対象外の既存事象。`nagoya-hitsumabushi.html` / `nagoya-solo-dining.html` に新規違反なし。写真は既存URLのみ使用し新規追加なし
  - **acceptance 6（効果測定のベースライン）**: 本チケット実装前の直近28日（2026-08-17〜09-13）実測値を効果測定の起点として記録する — **表示306 / クリック2 / CTR 0.65% / 平均順位27.2**（`data/gsc_metrics.json` の `pages` 配列）。4週間後に同ソースで前後比を確認し、`status` を `done`（改善確認）または現状維持の判断材料とする
  - **検証**: `node scripts/audit_design_system.js --check` で `nagoya-hitsumabushi.html`/`nagoya-solo-dining.html` に新規違反なし（既存の別ファイル群の違反は本チケット無関係）。`node scripts/add_feature_top_cta.js --check` で当ページ `"status":"unchanged"`。ローカルHTTPサーバでデスクトップ・モバイル両ビューポートを目視確認し、レイアウト崩れなし・新規リンクの遷移を確認済み
  - **判断**: 「勝てないと判明したら閉じる」ガードレールには該当しない。KWは正しく、内容・内部リンクの実力不足という**是正可能な原因**が特定できたため、rank_push施策を実施した。効果が出るかは4週間後のGSC実測で判定するため `status: in_progress` とする
...
- **2026-09-24 追記（日次triage・未反映のまま欠陥が継続・新顔KW「おひとりさま」を観測）**: 2026-09-23 分の助言🟡が `features/nagoya-solo-dining` に対し「名古屋 おひとりさま」「名古屋 一人ご飯」でのKW強化を提案した。**「おひとりさま」は本チケットがこれまで記録した0表示KW（一人ご飯／接待／ひつまぶし ランチ）に無い新顔**で、同じ欠陥が新しい語で再生産されている。実測（`data/gsc_metrics.json` 2026-09-23 生成・28日・上位300クエリ。誰でも GSC で再現可能）:

  | 助言が提案したKW | 実測 |
  |---|---|
  | 名古屋 おひとりさま | 「おひとりさま」を含むクエリ **0本 / 表示0** |
  | 名古屋 一人ご飯 | 「一人ご飯」「ひとりごはん」とも **0本 / 表示0**（2026-09-11 の観測から変化なし） |
  | （助言に一度も出ない実在KW） | 「一人飲み」**10本 / 表示845 / クリック59**（`名古屋 一人飲み` 289表示/32クリック/5.6位、`名古屋駅 一人飲み` 180表示/7クリック/**10.5位＝1ページ目の境界**） |

  - 同日の `node scripts/check_gas_deploy_health.js --record` は **not_deployed（確定観測3回連続・許容2回）**。acceptance④ のデプロイが未実施である事実と、本欠陥が継続している事実が同じ日のデータで一致した
...
- **brand-filter**: ✅ 適合 — CLAUDE.md「無人自動化の監視を設計するときの原則」③「『気づけるはず』を検知と数えない」に直接該当。順位操作・広告・マネタイズを一切伴わない運用健全化
- **検証できる事実**（誰でも Gmail で再現可能・制約10）:
  - `.claude/commands/seo-triage-weekly.md` と本ルーチンの規定クエリ `subject:"📊 NAGOYA BITES 週次レポート" newer_than:9d` は Gmail 既定で trash を除外するため、**2026-09-19 時点で 0件**を返す
  - 同じ窓を `週次レポート in:anywhere newer_than:40d` で引くと **5通ヒットし、5通すべての `labelIds` に `TRASH` が含まれる**（2026-08-16 / 08-23 / 08-30 / 09-06 / 09-13 送信分）。つまり「今週分が未着」ではなく「届いているが規定クエリの視界の外」
  - **原因は日次/週次の区別ではなく「メールの古さ」**（`日次レポート in:anywhere newer_than:8d` で確認）。日次レポートも同じ扱いを受けている:
    | レポート | 送信 | 現在のラベル |
    |---|---|---|
    | 日次 2026-09-18 | 09-18 23:07 UTC | `INBOX` |
    | 日次 2026-09-17 | 09-17 23:07 UTC | `INBOX` |
    | 日次 2026-09-16 | 09-16 23:07 UTC | **`TRASH`** |
    | 日次 2026-09-15 / 09-14 / 09-13 / 09-12 / 09-11 | 各 23:07 UTC | **全て `TRASH`** |
    | 週次 2026-09-13（最新） | 09-13 23:07 UTC | **`TRASH`** |
  - **これが日次アームだけ生き残っている理由**: 日次は `newer_than:2d` で引くため、まだ INBOX にある2日分をたまたま捕まえられている。週次は `newer_than:9d` ＝ 窓が trash 化の時点をまたぐため、**構造的に必ず 0件になる**。日次も trash 化が早まれば同じ穴に落ちる（＝日次アームも運任せで成立している）
  - 影響: 週次トリアージは「該当メール無し＝スキップ（正常終了）」と報告し続けるため、**空振りが正常系に偽装される**。`data/seo_advice_log.json` の `line-weekly` 最終処理は 2026-09-14（09-06〜09-12 分）で、以降の週次は取りこぼし予備軍
  - 関連: 同じ観測の取り違えで [[ISSUE-129]]（P1・GAS停止疑い）が誤って起票されていた。本日の再検証で日次8日分が欠番なしと判明し `wont_fix` に是正済み
- **acceptance**:
  1. **日次・週次の両アーム**の取得クエリを ISSUE-089 規律に揃える（日次が今生きているのは窓が短く trash 化前に捕まえているだけで、恒久的な担保ではない）（primary → `in:anywhere` sweep → 時間差リトライ）。0件は sweep を消化して初めて確定させる。手順の正本を更新する（`.claude/commands/*.md` は自己改変ブロックのため編集不可 → オーナーへの手動依頼、または `docs/` 側の手順書に集約して本ルーチンのプロンプトから参照する運用にする）
  2. なぜレポートが数日で TRASH に入るかを特定する（Gmail のフィルタ設定が第一候補。自己送信メールを一定期間後に削除する設定など。**確認・解除はオーナー本人の Gmail 操作が必要**でコード側から検証できない）。特定できたら手順書に記録する。週次だけの問題ではなく日次も同じ扱いを受けている点に注意
  3. 取りこぼした週次（2026-09-13 送信分以降で未処理のもの）を遡って triage する。`seo_advice_log.json` の重複検知が冪等性を担保するので、再処理しても二重起票にならない
  4. 「週次が N 週連続で処理されていない」をサーバ側で検知できるようにする（`gas-deploy-watchdog.yml` / `feedback-watchdog.yml` と同じ out-of-band 通知＝GitHub Issue → メール）。ログファイルの中だけで完結させない（ISSUE-084 原則2）
- **ブランドガードレール**: 検知は「検証できる事実」だけで行う（`seo_advice_log.json` の `line-weekly` 最終日付の鮮度＝エージェントが自己申告できない値）。週次が実際に来ない週に鳴らしてオオカミ少年化させない（ISSUE-084 原則6）
...
- **priority**: P2 → **status**: in_progress（acceptance②③④実装済み。効果判定は次回GSC前後比を待つため即doneにはしない）
- **detected**: 2026-09-07
- **category**: SEO / コンテンツ
- **owner**: Editor
...
- **2026-09-14 実装（acceptance ②③④・Editor）**: 展開先1本目として `features/meieki-hitori-nomi.html`（名古屋駅×一人飲み）を新設した。
  - **掲載店の実在検証（機械照合・架空店ブロック）**: 掲載6店は全て `data/stores.json` の店名で完全一致することを `node` スクリプトで機械照合済み（下記6店すべて `FOUND in data/stores.json`）。選定は「タグ欄に `1人OK` または `カウンター` が付く」という検証可能な事実だけを基準にし、自己申告のコピー文言では選ばない（制約10）。
    - 名駅立呑 おお島（立ち飲み・海鮮／タグ:カウンター／話題フラグ・編集部推薦あり）
    - 串 ぽろ衛門 本店（居酒屋／タグ:カウンター,テラス席,1人OK）
    - 酔いどれ 名古屋駅前店（居酒屋／タグ:1人OK）※`nagoya-solo-dining.html` と重複掲載だが本ページでは名駅徒歩2分・深夜2時までという別角度で記述
    - 焼肉 福 名駅西口店（焼肉・ホルモン／タグ:1人OK）
    - 炭火焼肉 スギモト 名古屋うまいもん通り広小路口店（焼肉・ホルモン／タグ:半個室,1人OK。おすすめポイント欄に「一人焼肉応援のお店です」と明記）
    - グルマンズ（焼肉・ホルモン／タグ:1人OK）
  - 写真は全店 LOCAL_STORES 既存の写真URL（HotPepper 由来5店＋Places user-tier 1店）をそのまま使用。Places user-tier（名駅立呑 おお島・写真出所:places-user・クレジット「Kチャップ」）はページ上に `📷 Kチャップ` のクレジット表示を付与（`data/photo_policy.json` の代替枠クレジット表示義務に対応）。新規画像・ストック写真は一切使用していない
  - 名駅の出口別ミニマップ（表組み）で「国際センター駅・近鉄名古屋駅周辺／名古屋駅6番出口／名駅中心部／西口／JR広小路口」の5エリアに整理し、`nagoya-solo-dining.html`（市内全域のシーン特集）・`meieki.html`（名駅のエリア特集）のどちらにも無かった「名駅×一人飲み」の交差軸を追加
  - acceptance④の相互リンク: `features/nagoya-solo-dining.html` の関連特集セクション先頭に本ページへのリンクを追加（既存の関連リンク形式に準拠）。`features/meieki.html` の関連特集セクションにも追加。本ページの関連特集からも両ページへ相互リンク
  - サイトへの登録: `features/index.html`（ItemList JSON-LD position 57 に追加・article-card を solo-dining カードの直後に追加）／`sitemap.xml`（`<url>` エントリ追加）。`data/featured.json`（月替わり showcase/featuredStrip）は月次編集判断のため今回は未登録（`nagoya-dining-professionals.html` 等の既存特集も同様に未登録のものが多く、必須の登録先ではないため）
  - 検証: `node scripts/audit_design_system.js --check`（新規ページの違反0件・既存の421件はベースラインで無関係）／`node scripts/audit_feature_stores.js`（新規ページ起因の実在不明掲載店0件）／`node scripts/build_featured.js --check`（既存の鮮度ガード継続通過）／ローカルHTTPサーバでの目視確認（ヘッダー・パンくず・ヒーロー写真・店舗カード写真・出口別テーブル・FAQ・関連リンク・フッターまで正常表示、店舗詳細リンク2件を実クリックで到達確認）
  - **効果測定は未実施**（GSCの前後比較には最低数週間のクロール期間が必要）。次回の週次/日次SEOトリアージで `名古屋駅 一人飲み`（現状10.5位）の表示・クリック・掲載順位の前後比を確認し、変化が無ければ「勝てないと判明した」として率直にクローズする（[[SEO-091]] のブランドガードレールと同じ扱い）。status は `in_progress` のまま据え置き、効果確認後に `done` へ

- **2026-09-19 追記（日次トリアージで実測・着手順の固定）**: 同旨の助言が日次で再来したため本チケットに集約した（原文「『nagoya-solo-dining』特集記事の冒頭に『一人飲みにも使える個室・接待向け店舗はこちら』などの導線を設置し、関連特集への遷移を促しましょう」）。助言が言う「導線が無い」は事実と異なるが、**「位置」については未対応の余地が実在する**ので事実だけ記録しておく。検証できる事実（誰でもファイルを開いて再現可能）:
...
- **2026-09-18 試行と revert（(1)は実写自体は解決済みと確認できたが、canonical ID昇格は事故を起こして取り消した）**: `data/manual_stores.json` の那古野しば福やレコードを確認したところ、`写真URL`・`写真出所:"hotpepper"`・`写真確認日`・`HP写真確認日`・`HotPepper照合ID:"J003671888"` が既に埋まっており、**実写自体は日次CI（`HOTPEPPER_API_KEY`を持つ環境）の写真照合パイプラインが既に解決済み**だったことは確認できた（本チケット記載時点の未完メモは実写に関しては古くなっていた）。そこで照合済みIDを canonical な `ホットペッパーID` フィールドへ昇格するコミットを一度 push したが、本番CI（`node build.js` が実際にHotPepper APIから当該地域を再取得する環境）で `単体・回帰テスト`（`tests/search_relevance.test.js` の「店名の部分一致が引き続き最上位に来る」）が **FAIL** し、build.yml のパイプライン全体（sitemap更新・写真昇格・各種監査・commit&pushを含む後続ステップ全て）が停止した（`Commit & push if changed` ステップまで `skipped` になったため、壊れたデータ自体はリポジトリにコミットされずに実害はゼロだったことは確認済み）。原因は未確定だが、`mergeManualStores()` が `ホットペッパーID` 一致を優先照合キーにする際「店名は既存優先」で manual 側の店名を採用しない設計のため、この店が実際のHotPepperスクレイプ結果と**別表記の同一店として衝突・店名が意図せず入れ替わった**可能性が高いと推測している（ローカルには `HOTPEPPER_API_KEY` が無く実地検証ができないため確定できない＝当初の未完メモが警告していた制約そのものだった）。**該当コミットは直後に revert 済み**（`ホットペッパーID` フィールドを追加した差分のみを取り消し、それ以外の変更は維持）。canonical ID昇格の再挑戦は、`HOTPEPPER_API_KEY` を持つ環境（オーナーのローカル or CI dispatch）で実地検証してから行うこと。実写自体（写真URL）は既存のまま解決済みで今回のrevertによる影響はない
...
- **acceptance（残り）**:
  ...
  4. ~~効果判定を `gsc_query_intent.js` の discovery クリックで行う（4週間後＝2026-09-14 に確認）~~ ⚠️ **2026-09-23実施（9日遅延・見落とされていた）**: `data/gsc_metrics.json`（直近28日）の `topPages` に `date.html`（291表示・9クリック・**12.5位**）が新規で入ってきた＝見出し修正前(13.8位)から悪化はしていないが劇的改善でもない。一方 **`banquet.html`・`private-room.html`・`nagoya-settai-secret.html` は今回もtopPages(15件)に一度も入らず**＝表示回数がそもそも上位15ページの足切り未満のまま、5週間経っても可視化不能。`nagoya-kaoawase-washoku.html` は表示75・27.3位とほぼ横ばい（施策前28.6位から実質変化なし）。**結論: 見出し階層(H2/H3)の追加だけでは大半のシーンKWは動かなかった**。「一人飲み」だけが機能する理由は他にもある可能性が高く、本チケットのacceptance②の特定は不十分だったと訂正する
  5. 施策前の数値を `data/effect_ledger.json` に記録して前後比で閉じる（**総クリックは指名検索と混ざるため使わない**・SEO-043 の判定器を流用） → 未実施のまま今回に至った（次回から同種の「N週間後に確認」系acceptanceは`data/effect_ledger.json`か`ScheduleWakeup`相当の仕組みで確実に想起できるようにすること。今回9日の見落としが発生した）
- **2026-09-23 追記（季節タイミングの構造的懸念 → オーナー承認のうえ実装済み）**: `data/featured.json` の `monthlyScenes` を確認したところ、**`banquet`（忘年会）がトップページの特集面に登場するのは11月から**（9月・10月のシーンには一度も含まれていない）だった。本チケット自身が「忘年会の検索需要は9月下旬から立ち上がる」「10月以降着手では今年は取れない」と明記していたにもかかわらず、**サイト自身の内部露出（ホームページ特集面・季節バナー）は11月からしか始まらない**設計になっていた。ただし上記4の実測が示すとおり「露出量（被リンク数）は banquet.html が solo-dining より元々多い」ことは既に反証済みの変数のため、monthlyScenes前倒しが実際に順位を動かすかは**未検証の仮説**にとどまる。
...
### [SEO-058] トップページが表示15,891を消費して38クリックしか生んでいない（順位18.1位・CTR 0.24%） ✅ Strategic Skip

- **priority**: P2 → **status**: done（Strategic Skipとして解決）
- **resolved**: 2026-09-23
- **resolved_by**: Orchestrator（acceptance①で実装したpage×query取得が実データとして揃ったため、②③の診断を完了）
- **2026-09-23 診断完了（acceptance②③）**: `data/gsc_metrics.json` の `pageQueries` で `/` の実クエリ15件を確認。**全15件が個別店舗名クエリ**（「ロジウラリューキュー」「bar 愚者」「bistro屋 拗」等、いずれも1〜8表示・24〜54位）で、`gsc_query_intent.js` の分類でも同型は navigational（店名の指名検索）に該当。`data/gsc_opportunities.json` の `byPage.rankPush` が挙げる「/」の upside 109.4クリックは、**表示回数(4,191)が数千件規模の長尾の店名クエリに1〜8件ずつ細かく分散した合計**であり、単一の是正可能なコンテンツ機会ではないと判断。既に自社の `stores/*.html` が同じクエリで8〜10位と有利に戦えている（[[SEO-050]]時点の既存知見どおり）ため、トップページ側を無理に押し上げる（＝自社の店舗ページと共食いさせる）施策は取らない。**acceptance②の判定基準どおりStrategic Skipとしてクローズ**（表示回数が今後さらに減っても害ではない）
- **旧status**: partial（acceptance①のコード修正のみ完了。②③の診断は実GSCデータ待ちで未着手）
...
- **2026-09-18 定点観測（自動ルーティン・/solve-next）**:
  `node scripts/audit_crosscheck_v3.js` 実行結果:
