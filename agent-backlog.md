...
- **2026-09-23 追記（季節タイミングの構造的懸念・要オーナー判断）**: `data/featured.json` の `monthlyScenes` を確認したところ、**`banquet`（忘年会）がトップページの特集面に登場するのは11月から**（9月・10月のシーンには一度も含まれていない）。本チケット自身が「忘年会の検索需要は9月下旬から立ち上がる」「10月以降着手では今年は取れない」と明記していたにもかかわらず、**サイト自身の内部露出（ホームページ特集面・季節バナー）は11月からしか始まらない**設計になっている。ただし上記4の実測が示すとおり「露出量（被リンク数）は banquet.html が solo-dining より元々多い」ことは既に反証済みの変数のため、monthlyScenes前倒しが実際に順位を動かすかは未検証の仮説にとどまる。低リスク（JSON日程の前倒しのみ・新規コンテンツなし）だが、9月・10月に予定している他シーン（秋グルメ・接待・ハロウィン女子会）を差し替えるか純増させるかは編集判断のため、実装前にオーナー確認を要する
- **判明した残件（別途対応が必要）**:
