...
### [ISSUE-128] Instagram公式埋め込みウィジェットが本番で機能しておらず、店舗ページの「公式Instagramの実際の写真」が写真ではなくテキストリンクのまま表示されている（サイト全体・原因未確定）

- **priority**: P1 → **status**: ready（原因の暫定切り分けまで完了。恒久対策は原因確定後）
- **detected**: 2026-09-15（[[複数投稿表示機能]] の本番確認中に、オーナーから「表示されないです」と報告。調査の結果、今回変更していない既存の単一投稿ページでも同一症状を確認し、本件を独立の課題として切り出した）
- **category**: 表示 / 外部連携（Instagram embed）
- **owner**: Builder / DataKeeper（写真ソース優先順の運用は両者共管・CLAUDE.md参照）
- **source**: 本セッションでの直接調査（本番URLをブラウザで開き、DOM・iframe状態・コンソールを実測）
- **検証できる事実（制約10・第三者が同じ手順で再現できるもの）**:

  | 事実 | 確認方法 |
  |---|---|
  | 本番 `https://nagoya-bites.com/stores/J004537202.html`（本タスクで4件表示に変更）で、Instagram埋め込みが写真ではなく「◯◯の公式Instagram投稿を見る」というフォールバックのテキストリンクのまま表示される | ブラウザで開いて目視 |
  | **今回一切変更していない既存の単一投稿ページ** `https://nagoya-bites.com/stores/J000015064.html` でも、全く同じ症状（フォールバックテキストのみ）を確認した | 同上。git blame でこのファイルが本タスクで無変更であることも確認済み |
  | `window.instgrm` は正常に読み込まれており（`typeof window.instgrm === 'object'`）、`window.instgrm.Embeds.process()` を呼ぶと `<blockquote>` は正しく `<iframe src="https://www.instagram.com/.../embed/...">` に変換される（＝サイト側のスクリプト読み込み・DOM変換ロジックは正常に動作している） | ブラウザの devtools で `document.querySelectorAll('iframe').length` 等を実行し確認 |
  | しかし生成された iframe は **`height: 0`** のまま変化しない（Instagram側の埋め込みページが、読み込み完了時に送るはずの postMessage による自動リサイズ通知を一切返していない） | 同上。`document.querySelector('iframe.instagram-media').height` |
  | 同時期に、同じくInstagram側の別エンドポイント（`https://www.instagram.com/api/v1/users/web_profile_info/`・投稿一覧取得用の匿名アクセス）が `{"message":"Please wait a few minutes before you try again.","require_login":true}` を返すようになっていることを確認済み（[[複数投稿表示機能]] の調査時） | `curl -H "x-ig-app-id: 936619743392459" "https://www.instagram.com/api/v1/users/web_profile_info/?username=<any>"` |
- **未確定（推定であり事実として断定しない）**: 上記の別エンドポイントの `require_login` 化と同じ、Instagram側の匿名/未ログインアクセスに対する制限強化が、埋め込みiframe自体の読み込み完了通知にも及んでいる可能性がある。ただしiframe内部のレスポンス本文はブラウザのCORS制約で直接読めておらず、確定はしていない。他の原因（CDN・キャッシュ・当該投稿個別の状態等）の可能性も排除できていない
- **影響範囲（未確定）**: `__igVerified` が true の全店舗（実測 概ね1,000件超のオーダー・[[ISSUE-089]] 所有者検証通過数を参照）で同様に表示されない可能性がある。個々の店で再現確認はしていない
- **次のアクション（未着手）**:
  1. 複数の異なる投稿・異なるアカウントで再現するか追加サンプリングする（1〜2店だけでなく無作為抽出で）
  2. 自分のInstagramアカウントにログイン済みの通常ブラウザ（cookie/セッションあり）で同じページを開いた場合に表示が変わるか確認する（＝ログイン有無が原因かの切り分け）。ログイン済みで直っていれば「サイト側の対処は不可・Instagram側の仕様変更を待つしかない」という結論になる
  3. Instagram/Meta公式の embed.js に関する既知の障害情報・changelogを確認する
  4. 原因確定後、代替表示（例: 埋め込みが一定時間で読み込まれない場合はテキストリンクを維持する現状のフォールバックのままにするか、別の見せ方に変えるか）を検討する
- **意図的にやらなかったこと**: 原因が未確定のまま、コード側で見た目だけを取り繕う対症療法（例: フォールバックリンクの装飾を変える等）はしていない。CLAUDE.md品質ゲート原則1「合否を分ける入力は後から第三者が確認できるものに限る」に基づき、確定していない推定は「未確定」と明記した
- **関連**: [[複数投稿表示機能]]（本課題発見のきっかけ・そちらのコード自体は正しく動作していることを確認済みで、本課題の原因が解消され次第、追加の変更なしに表示される見込み）/ [[ISSUE-089]]（Instagram埋め込みの所有者検証）
