...
- **オーナーがGSC画面で実データを開示（2026-08-20・追加確認）**: 「ページのインデックス登録」レポート全体は 未登録593件（理由6種）／登録済み5,520件。内訳は 代替ページ(canonical)205 / **404 180** / **リダイレクト5** / クロール済み未登録132 / 検出未登録65 / 重複6。このうち元メールが指摘した2種を実URLで裏付け:
  - **404の180件**: サンプル10件（ドリルダウン先頭ページ）全てを `git log --diff-filter=D` で追跡 → 全件 [[ISSUE-050]]（孤児ページ削除・2026-05-14）/ 2026-05-22の記事整理 / 2026-06-03「正規ワークフローで作り直し」のいずれかで**意図的に削除済み**と確認。推移グラフも2026-06-13に急増して以降ずっと横ばいで新規発生ではない。**対応不要**（410を返せないGitHub Pagesの制約上、Googleが時間経過で自然に「未知」から外すのを待つ以外の手段がない）
  - **リダイレクトの5件**: `http://www.nagoya-bites.com/`・`https://www.nagoya-bites.com/`・`http://nagoya-bites.com/`（→いずれも正規URL `https://nagoya-bites.com/` へ301。意図した正規化で**正常設計**）／`https://nagoya-bites.com/stores/`・`https://nagoya-bites.com/stores/index.html`（前回クロール2026/08/11時点ではリダイレクトだった可能性があるが、今curlで直接確認すると**現在は200・リダイレクトなし**）。**5件とも実害なし**
  → 元メールが指摘した2要因（リダイレクト・404）は両方とも実データで「現在は無害」と確定。残り4理由（代替ページ205／クロール済み未登録132／検出未登録65／重複6）はGoogleシステム起因または正常なcanonical処理で、いずれも今回のメールの指摘対象外
...
- **acceptance**: `node scripts/audit_sitemap_health.js --check` がローカルから本番URLへ実行でき、5,205 URL全件 200 を確認済み。CI（build.yml）に統合済み。GSC実データ（404 180件サンプル・リダイレクト5件全件）を個別に検証し実害ゼロと確認済み
...
| 2026-08-20 | Orchestrator(EXPLICIT) | ISSUE-100 追加検証 — オーナーがGSC画面から開示した実データ（未登録593件・理由6種の内訳）を基に、元メールが指摘した404 180件（サンプル10件をgit履歴で追跡し全件[[ISSUE-050]]等の意図的削除と確認）とリダイレクト5件（www/http系3件は正規化の正常設計、stores/系2件は現在curlで200確認・リダイレクト解消済み）を個別検証し、実害ゼロと確定 | ✅ 本コミット |
