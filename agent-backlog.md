...
- **検証できる事実**（誰でも Gmail で再現可能・制約10）:
  ...
  - 日次レポートは同じ GAS・同じ送信元・同じ宛先で `INBOX` に入っている（2026-09-18 分を本日取得済み）。**日次と週次で振り分けが分かれている**＝GAS 側ではなく受信側（Gmail のフィルタ等）の要因が濃厚
  - 影響: 週次トリアージは「該当メール無し＝スキップ（正常終了）」と報告し続けるため、**空振りが正常系に偽装される**。`data/seo_advice_log.json` の `line-weekly` 最終処理は 2026-09-14 で、以降の週次は取りこぼし予備軍
- **acceptance**:
  1. 週次アームの取得クエリを日次と同じ ISSUE-089 規律に揃える（primary → `in:anywhere` sweep → 時間差リトライ）。0件は sweep を消化して初めて確定させる。手順の正本を更新する（`.claude/commands/*.md` は自己改変ブロックのため編集不可 → オーナーへの手動依頼、または `docs/` 側の手順書に集約して本ルーチンのプロンプトから参照する運用にする）
  2. なぜ週次だけ TRASH に入るかを特定する（Gmail のフィルタ設定が第一候補。**確認・解除はオーナー本人の Gmail 操作が必要**でコード側から検証できない）。特定できたら手順書に記録する
