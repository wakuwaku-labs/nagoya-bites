...
### [ISSUE-128] Instagram公式埋め込みウィジェットが `X-Frame-Options: DENY` により全店舗で機能しなくなっている（写真ソース優先1が事実上使用不能）

- **priority**: P0 → **status**: ready（原因確定。恒久対策は写真ソース優先順の再設計を伴うため別途方針決定が必要）
...
- **原因特定の道筋（切り分けの実測）**:
  ...
  | 外部情報（Web検索）: 「2026年、Metaが匿名/埋め込みアクセスへの制限を強化した」という複数の独立ソースの記述 | 時期的に整合（裏付け情報であり、単独の根拠にはしていない） |
...
- **影響範囲**: `blockquote.instagram-media` + `embed.js` 方式で埋め込んでいる**全店舗ページ**（`__igVerified` が true の全件・[[ISSUE-089]] 所有者検証通過数を参照）。これは技術的にブラウザレベルで確定的に失敗するため、抽出サンプルではなく**悉皆的に**影響していると判断できる（同一メカニズムを使う限り例外は生じ得ない）
- **戦略的インパクト（Strategist判断が必要）**: CLAUDE.md「写真ソース優先順」の **優先1（公式Instagram embed）が事実上使用不能**になった。優先2（HotPepper写真）・優先4（Places写真）への実質的な依存度が全店舗で上がっている状態。優先順の記述そのものを見直すか、Instagram側の復旧を待つかはオーナー判断が必要
- **次のアクション**:
  1. オーナーへ状況共有し、優先順の扱い（当面優先2以降を実質優先とみなすか等）を確認する
  2. `X-Frame-Options: DENY` が恒久的な仕様変更か、一部投稿/一部時間帯だけの現象かを継続監視する（複数店・複数日で再チェック）
  3. 代替表示の検討: 埋め込みが一定時間で読み込まれない場合、現状の「フォールバックのテキストリンク」を保つか、別の見せ方（例: 優先2以降の実写のみを主役にし、Instagramは外部リンクボタンに統一する等）に変えるかを検討する。ただし変更は既存のUI/UXへの影響が大きいため、実装前にオーナー承認を得る（品質ゲート原則3）
- **意図的にやらなかったこと**: 原因確定前に、見た目だけを取り繕う対症療法はしていない。原因確定後も、優先順の再設計自体は本セッションの独断で行わず、オーナー判断を仰ぐ
