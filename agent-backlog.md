...
- **追加 acceptance**:
  5. `npm test` を CI（`build.yml`）で実行し、テストが実際に防壁として機能する状態にする。
     **ただし現在1件失敗しているため、先に上記の1/2の判断を済ませてから追加する**
     （赤いまま追加すると毎日のビルドが落ち続け、`continue-on-error` で無効化される未来になる）
  6. `tests/reel_ownership.test.js` のハンドル名ベタ書きを、条件から導出する形へ改める（陳腐化しない判定にする）
