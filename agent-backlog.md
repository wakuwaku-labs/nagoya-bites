...
### [SEO-089] 「予約行動 0.0人」は行動が無いのではなく**分子の定義が欠けている** — SEO-072 の acceptance ② が求めた「予約ドメインへの `outbound_click`」が実装されないまま done になり、🔴枠が毎日その 0 で埋まる
...
- **実害**: 「行動イベントが全く発生していません」という**事実に反する前提**が生成AIアドバイスの入力になり、🔴（最高深刻度）枠を消費して「FVを直せ」という同じ打ち手を再生産し続ける。[[SEO-075]] [[SEO-076]]（直帰率90%台の誤値が1週間🔴枠を占めた）と同一クラスの、助言枠の構造的な浪費
- **acceptance**:
