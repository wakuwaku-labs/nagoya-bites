...
- **owner**: 片桐 ← Builder / DataKeeper（写真ソース優先順の運用は両者共管・CLAUDE.md参照）/ Strategist（優先順再設計が必要なため）
...
- **影響範囲**: `blockquote.instagram-media` + `embed.js` 方式で埋め込んでいる**全店舗ページ**（`__igVerified` が true の全件・[[ISSUE-089]] 所有者検証通過数を参照）で同一症状が出ている（本セッションで検証した店舗全てで再現）。ただし上記の通り原因（意図的か選択的か）が未確定なため、「今後も一律に全店舗で起き続けるか」自体は断定できない
- **戦略的インパクト（Strategist判断が必要）**: CLAUDE.md「写真ソース優先順」の **優先1（公式Instagram embed）が少なくとも現時点では事実上使用不能**になっている。優先2（HotPepper写真）・優先4（Places写真）への実質的な依存度が上がっている状態。優先順の記述そのものを見直すか、原因確定・復旧を待つかはオーナー判断が必要
- **次のアクション**:
  1. オーナーへ状況共有し、優先順の扱い（当面優先2以降を実質優先とみなすか等）を確認する
  2. **原因の切り分け**: 実際にInstagramへログイン済みの一般的なブラウザ（自動化ツールでない、通常の閲覧環境）で本番ページを開き、同じ症状が出るか確認する。出なければ仮説2（ボット検知）が濃厚、出れば仮説1（一律の仕様変更）が濃厚
  3. `X-Frame-Options: DENY` が恒久的な現象か、一部投稿/一部時間帯だけの現象かを複数店・複数日で継続監視する
  4. 代替表示の検討: 埋め込みが一定時間で読み込まれない場合、現状の「フォールバックのテキストリンク」を保つか、別の見せ方（例: 優先2以降の実写のみを主役にし、Instagramは外部リンクボタンに統一する等）に変えるかを検討する。ただし変更は既存のUI/UXへの影響が大きいため、実装前にオーナー承認を得る（品質ゲート原則3）
- **意図的にやらなかったこと**: 「なぜ」が未確定のまま、見た目だけを取り繕う対症療法はしていない。優先順の再設計自体は本セッションの独断で行わず、オーナー判断を仰ぐ
- **関連**: [[複数投稿表示機能]]（本課題発見のきっかけ・そちらのコード自体は正しく動作していることを確認済みで、本課題が解消すれば追加の変更なしに表示される見込み）/ [[ISSUE-089]]（Instagram埋め込みの所有者検証）
- **files**: 変更なし（調査のみ。関連コードは `gen-store-pages.js` の `.ig-photos` セクション・全店舗の `stores/*.html`）
