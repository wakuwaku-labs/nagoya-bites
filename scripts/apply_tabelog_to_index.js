#!/usr/bin/env node
/**
 * apply_tabelog_to_index.js
 *
 * data/tabelog_resolved.json の解決済み公式食べログURLを、
 * index.html 内の LOCAL_STORES の 食べログURL フィールドにマージする。
 *
 * 使い方:
 *   node scripts/apply_tabelog_to_index.js                  # 適用
 *   node scripts/apply_tabelog_to_index.js --dry-run        # 適用せず差分のみ表示
 */
...
function main() {
  if (!fs.existsSync(TABELOG_CACHE)) {
    console.error(`キャッシュが存在しない: ${TABELOG_CACHE}`);
    console.error('先に node scripts/resolve_tabelog.js を走らせてください');
    process.exit(1);
  }
  const resolved = JSON.parse(fs.readFileSync(TABELOG_CACHE, 'utf8'));
  ...
  for (const s of stores) {
    ...
    if (!entry || !entry.tabelog || entry.failed) { noResolve++; continue; }
    if (s['食べログURL'] && s['食べログURL'].trim()) { skippedManual++; continue; }
    if (dryRun) {
      console.log(`  [${id}] ${s['店名']} → ${entry.tabelog}`);
    }
  ...
  console.log(`✓ index.html を更新（${applied}件の食べログ公式URLを焼き付け）`);
}
