 ...
 *   証跡: 採用した店には HotPepper店名 / HotPepperURL / HotPepper照合ID を書き戻すので、
 ...
 *
 * ※ 照合できた HotPepper ID は `HotPepper照合ID` に入れる。build.js の合流キーである
 *   `ホットペッパーID` に**あえて入れていない**理由は、書き込み箇所のコメントを参照
 *   （重複は解消できるが、生き残る店名が検索で引けなくなる既存バグを踏むため暫定回避）。
 */
const fs = require('fs');
...
async function main() {
  ...
  for (const s of targets) {
    ...
    if (DRY) continue;
    s['写真URL'] = url;
    // ⚠ ホットペッパーID は**あえて書かない**（2026-08-30・暫定）。
    //
    // このキーを書くと build.js の mergeManualStores が既存の HotPepper 店へ手動店を
    // 吸収し、重複が1件に片付く（本来は望ましい）。ところが吸収後に生き残る店名は
    // HotPepper 側の短い名前になり、その店が検索で自分の名前で引けないことが発覚した。
    // 原因は検索エンジンの正規化が長音符「ー」を記号として落とし、'バー'→'ば' という
    // 1文字の概念語を作って店名の途中から文字を奪うため（「し"ば"福や」→「し福や」）。
    // 同じ理屈で 'ビール'→'びる' が「◯◯ビル」＝建物にヒットする実害も出ている。
    //
    // 正規化の修正はサイト全体の検索結果を変える（実測で20クエリ中13件が変化）ため
    // 別PRで評価する。それまでは ID を書かず、重複解消を先送りして写真だけ入れる。
    // 照合できた ID は下の HotPepper照合ID に残すので、修正後にそのまま昇格できる。
    s['HotPepper照合ID'] = picked.shop.id || '';
    s['写真出所'] = 'hotpepper';
