...
function check() {
  ...
  // ---- 5. 参照値の鮮度チェック（SEO-075 acceptance 5）----
  // fetch_ga4_views.js が D-2 の確定参照値を CI が毎日書くはず。古いままなら CI が止まっている証拠。
  const staleMaxDays = Number.isFinite(w.max_stale_reference_days) ? w.max_stale_reference_days : 3;
  try {
    const sm = readJson(path.join(REPO, 'data', 'site_metrics.json'));
    const refDate = sm && sm.dailyReference && sm.dailyReference.date;
    if (refDate) {
      const staleDays = daysBetween(refDate, today);
      if (staleDays !== null && staleDays > staleMaxDays) {
        problems.push({
          kind: 'stale_reference',
          detail:
            `data/site_metrics.json の dailyReference.date が ${refDate}（${staleDays}日前・許容 ${staleMaxDays}日）。` +
            `scripts/fetch_ga4_views.js は D-2（2日前）の確定値を CI が毎日書くはずが、${staleDays - 2}日以上進んでいない。` +
            `.github/workflows/build.yml の最近の実行履歴を確認してください（SEO-075 acceptance 5）。`,
        });
      }
    }
  } catch (e) { /* site_metrics.json が読めない日は判定しない（鳴らさない側に倒す） */ }
