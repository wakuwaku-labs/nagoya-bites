...
// 業態に対して安すぎる予算をクリアする
// ────────────────────────────────────────────────────
// Hot Pepper の budget.name には、未設定店のデフォルト最低ランクや
// ランチ予算が紛れ込み、ディナー発見サイトとして非現実的な「～500円 /
// 501～1000円」が着席業態に付くことがある（例: 居酒屋・うなぎが ～500円）。
// 誤った価格を出すより「表示しない」方が信頼を損なわないため、業態を見て
// 明らかに低すぎるバケットは空にする。数値の捏造はしない（クリアのみ）。
//
// カフェ/スイーツ/パン等の軽食業態は ～500円（コーヒー1杯）が現実的なので対象外。
// ラーメンは 501～1000円 が通常価格なので「～500円」のみ非現実的とみなす。
const CHEAP_GENRES = ['カフェ', '喫茶', 'スイーツ', '甘味', 'パン', 'ベーカリー',
  'クレープ', 'ドリンク', 'ティー', 'サンド', '菓子', 'アイス', 'ジェラート',
  'ドーナツ', 'たい焼き'];
// 着席ディナー業態（一人あたり 1000円以下はランチ/未設定の誤データとみなす）
const DINNER_GENRES = ['居酒屋', '焼肉', 'ホルモン', '寿司', '鮨', '割烹', '料亭',
  '懐石', '会席', '鉄板', 'ステーキ', 'フレンチ', 'イタリアン', '焼鳥', '焼き鳥',
  'やきとり', 'うなぎ', '鰻', '日本料理', '和食', 'ダイニングバー', 'バル',
  'ビストロ', 'すき焼き', 'しゃぶしゃぶ', '韓国料理'];

function clearImplausiblePrice(bucket, genre) {
  const g = genre || '';
  if (bucket === '～500円') {
    // カフェ等の軽食以外で ～500円 は非現実的
    return CHEAP_GENRES.some(k => g.includes(k)) ? bucket : '';
  }
  if (bucket === '501～1000円') {
    // 着席ディナー業態で 501～1000円 は非現実的
    return DINNER_GENRES.some(k => g.includes(k)) ? '' : bucket;
  }
  return bucket;
}
...
function sanitizeStore(s) {
  ...
  // 業態に対して非現実的に安い予算（誤データ）はクリアして表示しない
  s['価格帯'] = clearImplausiblePrice(s['価格帯'], s['ジャンル']);
  return s;
}
...
async function main() {
  ...
  // 名古屋以外の誤掲載店舗を永続的に除外
  // ※Google Sheetsデータを修正するまでの恒久的な除外リスト
  const EXCLUDED_HP_IDS = new Set([
    'J004469034', // サザンクラウン（沖縄・竹富町）
    // ── 沖縄・那覇市（ゆいレール沿線） ──
    'J003862004', // やきとん居酒屋KIN-KAN（那覇市 ゆいレール安里駅）
    'J000721568', // 海人居酒屋 源 新栄店（那覇市 ゆらてぃく市場）
    'J003389352', // でんすけ商店 栄町店（那覇市 ゆいレール安里駅）
    'J004067277', // 焼肉韓国キッチン 琉球庵（那覇市 ゆいレール安里駅）
    'J001008132', // 焼肉 きたや 栄町店（那覇市 安里駅）
    'J001282792', // 栄ノ肉（那覇市 安里駅）
    // ── 東京都 ──
    'J001203202', // 魚雅 四ツ谷店（東京・四谷 JR中央線）
    'J003693344', // 日本栄光酒場 ロッキーカナイ 神楽坂（東京・飯田橋）
    // ── その他都市 ──
    'J001050388', // 真居酒屋 魁 らん 広島（広島市 広電胡町）
    'J003625220', // ひよこ酒場 仙台からおけ研究所（宮城・仙台 JR中野栄駅）
    'J003872812', // 焼肉 和牛のたか 熊本新市街店（熊本市 辛島町駅）
    'J001294910', // 焼肉 精肉直売所 六花 長岡店（新潟・長岡市）
    'J003916849', // バイキング左近 寝屋川店（大阪・寝屋川市）
    'J004469059', // 焼肉いやさか（大阪・古市駅）
    // ── ISSUE-103（2026-08-22）: places_resolved.json の rejected 分析で判明した71店 ──
    // fetch_places.js の地名寄せ誤りで他都道府県の同名チェーン支店IDが混入。
    // 正しい名古屋支店IDは次回 HotPepper API 収集時に自動補完される。
    'J003625289', // 魚々路 ととろ 釧路栄町店（北海道）
    'J001142608', // 旬酔かなで（熊本県）
    'J004559267', // New Normal Cafe 鴻巣店（埼玉県）
    'J000558465', // ほく菜 Hokusai 春吉本店（福岡県）
    'J000015780', // くいもの屋わん茅ヶ崎店（神奈川県）
    'J003736999', // またふく 九条店（大阪府）
    'J000280674', // 寧々家 多賀城店（宮城県）
    'J003991005', // やきとり居酒屋サカエ（宮崎県）
    'J001136340', // くいもの屋わん 大橋西口店（福岡県）
    'J001100256', // ミライザカ ルルサス防府店（山口県）
    'J004536976', // たこ焼き居酒屋 零（北海道）
    'J004577064', // KITCHEN COCORU（宮城県）
    'J000647995', // 浪花屋 鳥造 倉敷連島店（岡山県）
    'J001109961', // 沖縄料理 那覇 居酒屋 やーじぐゎー（沖縄県）
    'J001294624', // 本格炭火焼鳥 いち稟 二俣川駅店（神奈川県）
    'J003985041', // 炭火焼 居酒屋月光浴（福岡県）
    'J003693334', // 日本栄光酒場ロッキーカナイ高円寺（東京都）
    'J000517230', // 朝まで屋 テラス102（岡山県）
    'J003873133', // 居酒屋栄ちゃん鏡原店（沖縄県）
    'J001274343', // 海鮮炉端釧路っ子（北海道）
    'J003872780', // なはまぐろ 海鮮料理 居酒屋・マグロ専門店 ぎょぎょ丸 久茂地店（沖縄県）
    'J001127482', // 焼鳥が止まらない店 焼き膳 彩鳥（東京都）
    'J003532982', // ともり 明松（北海道）
    'J004068251', // 炭火やきとり×鮮魚 千鳥 本郷台店（神奈川県）
    'J000525871', // お好み焼 ･ 鉄板焼 徳川 海田店（広島県）
    'J003497714', // この島の大地（大阪府）
    'J003984782', // 居酒屋やま爺 栄（高知県）
    'J000629519', // もも（徳島県）
    'J000796240', // ヤン衆漁場二代目 昌栄丸（北海道）
    'J001038389', // ゆるりと菜 村さ来 茅ヶ崎店（神奈川県）
    'J000748737', // リビング & ダイニング hinode（沖縄県）
    'J003873090', // 築地海鮮酒場三代目ISSEI（埼玉県）
    'J000101693', // 會津っこ（福島県）
    'J001041305', // ワラッテ（岐阜県）
    'J003366284', // 海鮮居酒屋 築地市場298 西新井店（東京都）
    'J001250624', // 陽なた（兵庫県）
    'J004402409', // 大衆酒場 春田屋 江古田店（東京都）
    'J003649419', // 個室居酒屋 牛タンとおでん 山崎 八重洲本店（東京都）
    'J003736022', // 焼肉ホルモン さわだ（宮城県）
    'J004509941', // 七輪焼肉安安鹿沼店（栃木県）
    'J003433552', // 焼肉・ホルモン・韓国料理 はんぢゃん新発田店（新潟県）
    'J003917353', // らーめんや 煌/幻の焼肉店煌/キラッキー水産（新潟県）
    'J003762676', // カジュアルジビエ酒場イノシカブーム那覇店（沖縄県）
    'J003737128', // 黒毛和牛焼肉 はるひ（兵庫県）
    'J001219416', // 虎乃門 北光店（北海道）
    'J003474491', // 茅ヶ崎焼肉ホルモンたけ田（神奈川県）
    'J000029776', // 焼肉・ちゃんこ鍋 牛膳（ぎゅうぜん）（徳島県）
    'J001152746', // 肉どろぼう（広島県）
    'J000252543', // 炭火焼肉 島田屋 本店（北海道）
    'J000165574', // 安楽亭 羽村店（東京都）
    'J003324788', // 玄海南 三番町店（愛媛県）
    'J004025394', // 蜂屋（北海道）
    'J001282332', // 肉處 六（大阪府）
    'J003780001', // 赤身焼肉チャンピオン(旧ホルモン焼肉酒場チャンピオン)（長崎県）
    'J000757072', // 牛肉卸問屋直営 焼肉ホルモン八重山おときち（沖縄県）
    'J000903344', // つるみ（北海道）
    'J001280678', // 栄亭 綾部本店（京都府）
    'J000442750', // 栄亭 舞鶴店（京都府）
    'J001041205', // ぜんていの焼肉うし公 加茂店（新潟県）
    'J000778613', // ほるもん道場会津清水通店（福島県）
    'J001135626', // すたみな太郎 寝屋川店（大阪府）
    'J000796610', // 焼肉 おもに亭 アリオ西新井店（東京都）
    'J001209423', // サムライクラシック（愛媛県）
    'J000715131', // 大衆居酒屋 ごっちん（広島県）
    'J001161829', // 北海道イタリアン居酒屋 エゾバルバンバン 大通店（北海道）
    'J000823174', // 焼肉 まつおか（岡山県）
    'J000568001', // 揚屋（福岡県）
    'J000430789', // 飛騨牛焼肉 赤べこ 早田店（岐阜県）
    'J001060001', // 居酒屋 よいち 近鉄四日市店（三重県）
    'J001198210', // おばんざい ふじまさ（三重県）
    'J001266837', // 千の台所 津駅店（三重県）
  ]);
  ...
  let placesHistory = {};
  const placesHistoryPath = path.join(__dirname, 'data', 'places_history.json');
  if (fs.existsSync(placesHistoryPath)) {
    try {
      placesHistory = JSON.parse(fs.readFileSync(placesHistoryPath, 'utf8'));
      console.log(`places_history.json: ${Object.keys(placesHistory).length}件分の月次履歴を読込`);
    } catch (e) {
      console.warn(`places_history.json の読み込み失敗: ${e.message}（中立スコアで継続）`);
      placesHistory = {};
    }
  } else {
